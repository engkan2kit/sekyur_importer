...
grouping_id = 1
sekyur_curs.execute("INSERT INTO public.grouping (id, name) select %s, %s WHERE NOT EXISTS (SELECT 1 FROM public.grouping WHERE name=%s)", 
...
sekyur_curs.execute("INSERT INTO public.server (id, location, host_addr, port, \"user\", password) select %s, %s, %s, %s, %s, %s WHERE NOT EXISTS (SELECT 1 FROM public.server WHERE location=%s)", 
        (server_id, 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN')
    )
conn.commit()
